<?php
    ...
    if( overlap_has_title_area() ):
        ...
        if( empty($title_align) ) $title_align =  overlap_get_option('title_align');
        if( $title_align != 'none' ) $classes[] = 'text-'. $title_align;
        
        $title_size = get_post_meta( $page_id, '_w_title_size', true );
        if( empty($title_size) ) $title_size =  overlap_get_option('title_size');
        $classes[] = 'w-size-'. $title_size;
    
        $title_color = get_post_meta( $page_id, '_w_title_color', true );
        if( empty($title_color) ) $title_color = overlap_get_option('title_color');
        ...
        if($title_bg != ''){ //title background
            ...
            $title_bg = overlap_get_option('title_background_mode');
            ...
            if($title_bg !== 'none'){   
                ...
                if($title_bg == 'image'){   
                    ...
                    $title_bg_image = overlap_get_option('title_background_image');
                    if( is_array( $title_bg_image ) && isset( $title_bg_image['background-image'] )){
                        $title_bg_image = $title_bg_image['background-image'];
                    }
                    
                    $title_bg_image_size = overlap_get_option('title_background_image');
                    ...
                    $title_bg_effect = overlap_get_option('title_background_effect');
                    ...
                    $title_bg_video = overlap_get_option('title_background_video');
                ...
                $title_bg_color = overlap_get_option('title_background_color');
                
                $title_overlay_color = overlap_get_option('title_overlay_color'); 
                $title_overlay_opacity = overlap_get_option('title_overlay_opacity'); 
            }
        ...
        if( $title_scroll_effect == '') $title_scroll_effect = overlap_get_option('title_scroll_effect');
        ...
        $container_class = overlap_get_option('header_fullwidth') ? ' w-full' : '';
?>
<div<?php echo overlap_get_attributes( $attrs );?>>
    <?php if( !empty($title_bg_image) || !empty($title_bg_video) ): ?>
    <div class="title-background">
        <div<?php echo overlap_get_attributes( $background_attrs );?>>
    ...
    <div class="container<?php echo esc_attr( $container_class ); ?>">
        ...
        if( is_search() ) {
            ...
            $sub_title = esc_html__('Search Results for', 'overlap') .' : <strong>'. get_search_query().'</strong>';
		}

        if( is_404() ) {
			$title = esc_html__('Error 404', 'overlap');
            $sub_title = esc_html__('Page not found', 'overlap');
		}

        if( is_archive() && !is_woocommerce() ) {
			    
            if ( is_day() ) {
				$title = esc_html__( 'Daily Archives', 'overlap' );
                $sub_title = get_the_date();
			} else if ( is_month() ) {
				$title = esc_html__( 'Monthly Archives', 'overlap' );
                $sub_title = get_the_date('F Y');
			} elseif ( is_year() ) {
				$title = esc_html__( 'Yearly Archives', 'overlap' );
                ...
                if( empty($description) ) {
                    $sub_title = sprintf( esc_html__( '%s has created %s entries', 'overlap' ), $author->display_name, count_user_posts( $author->ID ) );
                }   
			} else {
				$title = single_cat_title( '', false );
                $sub_title = esc_html__('Currently browsing:', 'overlap') .' <strong>'. single_cat_title( '', false ) .'</strong>';
			}
        ...
        ?>
        <?php if($title_size != 's' && !empty($sub_title) ): ?>
        <h6 class="subtitle"><?php echo wp_kses_post( $sub_title );?></h6>
        <?php endif; ?>
        <h1 class="title">
