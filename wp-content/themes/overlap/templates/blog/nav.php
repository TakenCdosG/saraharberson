<?php
    ...
    if( overlap_get_option('blog_home') ) {
    
        if( overlap_get_option('blog_home_page') == 'default'){
            if( get_option( 'show_on_front' ) == 'page' )
                $home_button_url = get_permalink( get_option('page_for_posts' ) );
            else 
                $home_button_url =  home_url();
        }else{
            $home_button_url = overlap_get_option('blog_home_url');            
        }
...
<nav class="post-nav clear">
    ...
    <?php
		if($previous){
            $prev_thumbnail = overlap_get_post_thumbnail($previous->ID);
    ...
    <?php
		if($next){
            $next_thumbnail = overlap_get_post_thumbnail($next->ID);
            next_post_link('%link', '');
            echo '<div class="post-link clear">';            
