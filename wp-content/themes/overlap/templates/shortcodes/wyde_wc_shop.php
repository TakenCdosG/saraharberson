...
if ( $products->have_posts() ) : ?>
	...
	<p class="woocommerce-result-count">
		...
		$last     = min( $total, $products->get( 'posts_per_page' ) * $current_page );
		...
		if ( 1 == $total ) {
			...
			printf( esc_html( _x( 'Showing %1$d&ndash;%2$d of %3$d results', '%1$d = first, %2$d = last, %3$d = total', 'woocommerce' ) ), $first, $last, $total );
		}
		?>
	</p>
	...
	<div<?php echo overlap_get_attributes( $attrs );?>>			
	    ...
	    if( $pagination != 'hide' ){
	        overlap_pagination($pagination, $products->max_num_pages, $current_page); 
	    }
	    ?>			
